    ...
    else
    {
        if($phone_number == "all")
        {
            ...
            while($start_time->getTimestamp() <= $end_time->getTimestamp())
            {
                ...
                $sql = "select dailing_number as phone_number,  sum(cost) as cost from call_record where start_time > '".$start_time->format("Y-m")."' and end_time < '".$start_time->add(new DateInterval('P1M'))->format("Y-m")."';";
