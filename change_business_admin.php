...
//change_business_admin.php
    ...
    {
        $sql = "select free_message_count, free_flow_count from phone_number, combo, message_business, flow_business where phone_number.combo_id = combo.combo_id and combo.message_business_id = message_business.message_business_id and combo.flow_business_id = flow_business.flow_business_id and phone_number.phone_number = '".$_POST["phone_number"]."';";
        ...
        $sql = "SELECT * FROM number_business WHERE phone_number = '".$_POST["phone_number"]."';";
        $res = mysqli_query($mysqli, $sql);
        $res_array = mysqli_fetch_array($res, MYSQLI_ASSOC);
        if(empty($res_array))
        {
            $sql = "INSERT INTO number_business VALUES('".$_POST["phone_number"]."', ".$message_business_id.", ".$flow_business_id.");";
        }
        else
        {
            if(($message_business_id == NULL) && ($flow_business_id == NULL))
                $sql = "delete from number_business where phone_number = '".$_POST["phone_number"]."';";
            else
                $sql = "UPDATE number_business SET message_business_id = ".$message_business_id.", flow_business_id = ".$flow_business_id." WHERE phone_number = '".$_POST["phone_number"]."';";
        }
        $res = mysqli_query($mysqli, $sql);
        if($res == FALSE)
        {
        ...
        $sql = "update phone_number set message_count_per_month = ".$total_free_message_count.", flow_count_per_month = ".$total_free_flow_count." where phone_number = '".$_POST["phone_number"]."';";
        $res = mysqli_query($mysqli, $sql);
        if($res == FALSE)
        {
            printf("Update error!\n%s\n", mysqli_error($mysqli));
        ...
        header("Location: change_business_successfully_admin.html");
        mysqli_close($mysqli);
    }
?>
